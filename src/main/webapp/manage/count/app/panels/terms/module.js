          ...
          _.each(scope.results.aggregations.terms.buckets, function(v) {
            ...
            if(scope.panel.tmode === 'terms') {
                if(scope.panel.field==="运营大区ID" ||scope.panel.field==="渠道ID" ||scope.panel.field==="获得途径" ||scope.panel.field==="消耗途径" || scope.panel.field==="日志道具id" || scope.panel.field==="功能编号" || scope.panel.field==="注册渠道"){
                    slice = { label : map[v.key] , name : v.key, data : [[k,v.doc_count]], actions: true};
                }else if(scope.panel.field==="服务器ID"){
                      if(game === "fb"){
                        slice = { label : "fb_s"+v.key, name : v.key, data : [[k,v.doc_count]], actions: true};
                    }else if(game === "kds"){
                  ...
                  if(v.key=='1'){
                  slice = { label : '人民币',  name : v.key, data : [[k,v.doc_count]], actions: true};
                  }else if(v.key=='2'){
                  slice = { label : '美元',  name : v.key, data : [[k,v.doc_count]], actions: true};
                  }else{
                    slice = { label : v.key,  name : v.key, data : [[k,v.doc_count]], actions: true};
                    }
                }else if(scope.panel.field==="登录设备系统"){
                  if(v.key=='1'){
                    slice = { label : 'android',  name : v.key, data : [[k,v.doc_count]], actions: true};
                    }else if(v.key=='2'){
                    slice = { label : 'ios',  name : v.key, data : [[k,v.doc_count]], actions: true};
                    }else if(v.key=='0'){
                    slice = { label : '未知登陆设备',  name : v.key, data : [[k,v.doc_count]], actions: true};
                    }else{
                    slice = { label : v.key,  name : v.key, data : [[k,v.doc_count]], actions: true};
                    }
                  }else if(scope.panel.field==="是否首次登录"){
                  if(v.key=='0'){
                      slice = { label : '老用户',  name : v.key, data : [[k,v.doc_count]], actions: true};
                  }else if(v.key=='1'){
                      slice = { label : '新用户',  name : v.key, data : [[k,v.doc_count]], actions: true};
                    }else{
                      slice = { label : v.key,  name : v.key, data : [[k,v.doc_count]], actions: true};
                    }
                  }else{
            ...
            if(scope.panel.tmode === 'terms_stats') {
                if(scope.panel.field==="运营大区ID" ||scope.panel.field==="渠道ID" ||scope.panel.field==="获得途径" ||scope.panel.field==="消耗途径" || scope.panel.field==="日志道具id" || scope.panel.field==="功能编号" || scope.panel.field==="注册渠道"){
                      slice = { label : map[v.key],  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                }
                else if(scope.panel.field==="服务器ID"){
                      if(game === "fb"){
                        slice = { label : "fb_s"+v.key,  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                    }else if(game === "kds"){
                  ...
                  slice = { label : v.key+'级',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                }else if(scope.panel.field==="支付货币"){
                  if(v.key=='1'){
                      slice = { label : '人民币',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                      }else if(v.key=='2'){
                      slice = { label : '美元',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                    }else{
                    slice = { label : v.key,  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                    }
                }else if(scope.panel.field==="登录设备系统"){
                  if(v.key=='1'){
                      slice = { label : 'android',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                      }else if(v.key=='2'){
                      slice = { label : 'ios',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                      }else if(v.key=='0'){
                      slice = { label : '未知登陆设备',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                      }else{
                        slice = { label : v.key,  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                        }
                  }else if(scope.panel.field==="是否首次登录"){
                  if(v.key=='0'){
                      slice = { label : '老用户',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                      }else if(v.key=='1'){
                      slice = { label : '新用户',  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                    }else{
                    slice = { label : v.key,  name : v.key, data : [[k,v['subaggs'][_fcm[scope.panel.tstat]]]], actions: true};
                  }
                }else{
