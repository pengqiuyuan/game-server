...
public class XyjPayPenetrationScheduled {
	
	@Autowired
	private Client client;
	
	private static final Logger logger = LoggerFactory.getLogger(XyjPayPenetrationScheduled.class);
	
	//项目名称
	private static final String game = "XYJ";
	
	private static final String index_user = "logstash-xyj-user-*";
	
	private static final String index_money = "logstash-xyj-money-*";
	
	private static final String type_user = "xyj_userlog";
	
	private static final String type_money = "xyj_moneylog";
	
	
	private static final String bulk_index = "log_xyj_money";
	
	private static final String bulk_type_money_arpuday = "xyj_money_arpu_day";
	
	private static final String bulk_type_money_arpumouth = "xyj_money_arpu_mouth";
	
	private static final String bulk_type_money_arppuday = "xyj_money_arppu_day";
	
	private static final String bulk_type_money_arppumouth = "xyj_money_arppu_mouth";
	...
	public void testdayserverzone() throws IOException, ParseException {	
		...
		for (Terms.Bucket e : g1.getBuckets()) {
			 ...
			 mapsum.put((String) e.getKey(), agg.getValue());
		}
		...
		for (Terms.Bucket e : g2.getBuckets()) {
			...
			mapnum.put((String) e.getKey(), peoplen.getValue());
		}
		...
		for (Terms.Bucket e : g3.getBuckets()) {
			Cardinality ac = e.getAggregations().get("agg");
			 mapac.put((String) e.getKey(), ac.getValue());
		}
	...
	public void testmouthall() throws IOException, ParseException {	
		...
		SearchResponse paysum = client.prepareSearch(index_money).setSearchType("count").setTypes(type_money)
		        .setQuery(
		        		QueryBuilders.boolQuery()
		        		.must( QueryBuilders.rangeQuery("@timestamp").from(esUtilTest.presentfirstday()).to(esUtilTest.lastmouthfirstday()))
		        		.must( QueryBuilders.termsQuery("日志分类关键字", "money_get"))
		        		.must( QueryBuilders.termsQuery("支付货币", paytype))
		        )
		        .addAggregation(
	...
	public void schedual() throws Exception{
		logger.info("----------------xyj ARPU(day) ARPU(month) ARPPU(day) ARPPU(month) begin---------------");
		try {
			testdayall();
		} catch (Exception e) {
			// TODO: handle exception
			logger.debug("xyj all ARPU(day) ARPPU(day) error " + e);
		}
		...
		try {
			...
			// TODO: handle exception
			logger.debug("xyj serverZone ARPU(day) ARPPU(day) error " +e);
		}
		...
		try {
			...
			// TODO: handle exception
			logger.debug("xyj platForm ARPU(day) ARPPU(day) error " +e);
		}
		...
		try {
			...
			// TODO: handle exception
			logger.debug("xyj server ARPU(day) ARPPU(day) error "+e);
		}
		...
		try {
			...
			// TODO: handle exception
			logger.debug("xyj all ARPU(month)  ARPPU(month) error " +e);
		}
		...
		try {
			...
			// TODO: handle exception
			logger.debug("xyj serverZone ARPU(month)  ARPPU(month) error " + e);
		}
		...
		try {
			...
			// TODO: handle exception
			logger.debug("xyj platForm ARPU(month)  ARPPU(month) error " + e );
		}
		...
		try {
			...
			// TODO: handle exception
			logger.debug("xyj server ARPU(month)  ARPPU(month) error " +e);
		}

		logger.info("----------------xyj ARPU(day) ARPU(month) ARPPU(day) ARPPU(month) end--------------");
	}
