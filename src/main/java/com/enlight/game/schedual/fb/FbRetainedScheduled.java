...
public class FbRetainedScheduled {
	...
	private static final Logger logger = LoggerFactory.getLogger(FbRetainedScheduled.class);
			...
			if(dateBucket.equals(esUtilTest.twoDayAgoFrom())){
				...
				bulkRequest.add(client.prepareIndex(bulk_index, bulk_type_retained)
				        .setSource(jsonBuilder()
					           	 .startObject()
			                        .field("date", esUtilTest.twoDayAgoFrom().split(" ")[0])
			                        .field("gameId", game)
				...
				bulkRequest.add(client.prepareIndex(bulk_index, bulk_type_retained)
				        .setSource(jsonBuilder()
					           	 .startObject()
			                        .field("date", esUtilTest.eightDayAgoFrom().split(" ")[0])
			                        .field("gameId", game)
				...
				bulkRequest.add(client.prepareIndex(bulk_index, bulk_type_retained)
				        .setSource(jsonBuilder()
					           	 .startObject()
			                        .field("date", esUtilTest.thirtyOneDayAgoFrom().split(" ")[0])
			                        .field("gameId", game)
