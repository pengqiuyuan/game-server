...
public class FbPayConversionTest extends SpringTransactionalTestCase{
	...
	public void esAll() throws IOException, ParseException {	
		...
		if(dayPayRate_user.getHits().totalHits() == 0){
			...
			day = (double)dayPayRate_money.getHits().totalHits()/dayPayRate_user.getHits().totalHits();
		}
		...
		if(weekPayRate_user.getHits().totalHits() == 0){
			...
			week = (double)weekPayRate_money.getHits().totalHits()/weekPayRate_user.getHits().totalHits();
		}
		...
		if(mouthPayRate_user.getHits().totalHits() == 0){
			...
			mouth = (double)mouthPayRate_money.getHits().totalHits()/mouthPayRate_user.getHits().totalHits();
		}
